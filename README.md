...
| `SECRET_KEY` | `white-panel-secret-key-v2` | Session / hashing secret — **set a strong random one** |
| `RAILWAY_PUBLIC_DOMAIN` | — | Auto‑set by Railway; used as the real panel domain in configs |
| `DATA_DIR` | `/data` | Persistence dir for `white_state.json` |
...
5. Click **Connect & Deploy**. The panel:
   ...
   - Creates a KV namespace (`white-worker-kv`) and binds it to the Worker
...
data/              white_state.json, scanned/ (gitignored)
```
